...
const handlePurchase=()=>{
    ...
    return (
        ...
        <div className={detail!==0 ? "detailDiv" : null}>
            ...
            {isPurchase ? <Link to='/cart' style={{textDecoration:'none', color:'inherit'}}><PurchaseBtn fn={handlePurchase} text={"PURCHASE CART"}/></Link> : <ItemCount className='itemCount' fn={addNewProduct} itemData={detail}/>}
            {isPurchase ? <Link to='/' style={{textDecoration:'none', color:'inherit'}}><PurchaseBtn fn={handlePurchase} text={"GET MORE PRODUCTS!"}/></Link> : null}
            </div>
        </Details>
    )
}
